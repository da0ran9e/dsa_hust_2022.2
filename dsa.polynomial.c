...
void sort(int index){
	for(int i=0; i<terms[index]-1; i++){
		for (int j=0; j<terms[index]-i-1; j++){
			if(poly[index][i+1].e> poly[index][j+1].e){
				Terms temp = poly[index][i+1];
				poly[index][i+1] = poly[index][j+1];
				poly[index][j+1] = temp;
...
int main( ){
	...
	do{
		...
		if(strcmp(command, "Create")==0 ){
			...
			if(exist[poly_id]!=1) exist[poly_id]=1;
			//printf("d %d %d %d \n", poly[poly_id][terms[poly_id]].c, poly[poly_id][terms[poly_id]].e, terms[poly_id]);
		}
